...
const Projects = () => {
  ...
  return (
    <section id="projects" className="py-20 px-6 relative z-10">  {/* removed bg-[#0A192F] */}
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {visibleProjects.map(proj => (
            <div key={proj.id} className="glass-card rounded-xl overflow-hidden transition-all duration-300 glow-hover">  {/* replaced bg with glass-card */}
              <div className="h-48 bg-gradient-to-br from-[#1A2A3A] to-[#0A192F] relative group cursor-pointer" onClick={() => setSelectedProject(proj)}>
                <img src={`/assets/${proj.images[0]}`} alt={proj.title} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="bg-[#facc15] text-[#0A192F] px-4 py-2 rounded-lg text-sm font-medium">Click to enlarge</span>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-xl font-semibold text-gray-200 mb-2">{proj.title}</h3>
                <p className="text-gray-400 text-sm mb-4">{proj.desc}</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {proj.tech.slice(0, 4).map(t => (
                    <span key={t} className="px-3 py-1 bg-[#1A2A3A]/80 text-[#facc15] text-xs rounded-full border border-[#facc15]/20">{t}</span>
                  ))}
                </div>
                <ul className="space-y-1 mb-6">
                  {proj.features.slice(0, 2).map(f => (
                    <li key={f} className="text-gray-400 text-xs flex items-start"><span className="text-[#facc15] mr-2">•</span>{f}</li>
                  ))}
                </ul>
                <a href={proj.github} target="_blank" rel="noreferrer" className="inline-flex items-center px-4 py-2 bg-[#facc15] text-[#0A192F] text-sm font-medium rounded-lg hover:shadow-[0_0_15px_rgba(250,204,21,0.5)] transition-all duration-300 group">
        ...
        {projectsData.length > 3 && (
          <div className="text-center mt-12">
            <button onClick={() => setShowAll(!showAll)} className="inline-flex items-center px-6 py-3 bg-transparent border-2 border-[#facc15] text-[#facc15] font-medium rounded-lg hover:bg-[#facc15] hover:text-[#0A192F] transition-all duration-300 group">
